...
from gapmodel.asia import (
    breadth,
    build_asia_dashboard,
    constituent_table,
    driver_table,
    index_metrics,
    index_source,
...
from gapmodel.asia_report import render_asia_html, render_asia_text
...
def test_snapshot_and_rendering(panel):
    from gapmodel import asia as asia_mod

    single = asia_mod.snapshot(TOKYO, panel, influences=(DRIVER, LOCAL))
    assert single.source == "^TEST"
    assert set(single.constituents["symbol"]) == {"BIG.T", "SMALL.T"}

    built = asia_mod.AsiaDashboard(generated=pd.Timestamp("2026-01-01"), asia=[single])
    text = render_asia_text(built)
    assert "Test index" in text and "Big Co" in text
    html = render_asia_html(built)
...
def test_build_dashboard_needs_an_asian_index():
    with pytest.raises(RuntimeError):
        build_asia_dashboard({"BIG.T": bars(200)})
